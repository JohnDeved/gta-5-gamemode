    ...
    if (p_allPlayers != null) {
        for (var i = 0; i < p_allPlayers.Count; i++) {
            if (p_allPlayers[i] != null) {
				...
				var head_pos = API.getEntityPosition(player);
				...
				skel_head = API.worldToScreen(new Vector3(head_pos.X,head_pos.Y,head_pos.Z + 1));

				if(API.returnNative('HAS_ENTITY_CLEAR_LOS_TO_ENTITY', 8, API.getLocalPlayer(), player, 17) && distance < 15) {
